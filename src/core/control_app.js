...
function updateSceneSelectors(scenes) {
  ...
  targetSceneSelect.innerHTML = '<option value="">-- Select Scene --</option>' +
    scenes.map(scene => 
  ...
  returnSceneSelect.innerHTML = '<option value="">-- Keep Current Scene --</option>' +
    scenes.map(scene => 
...
function updateMonitorScenesList(scenes) {
  ...
  monitorList.innerHTML = scenes.map(scene => {
  ...
  try {
    ...
    if (dskStatusEl) {
      dskStatusEl.textContent = dskStatus.hasDSK 
        ? (dskStatus.dskActive ? '🟢 Active' : '🔴 Inactive')
        : '❌ Not found';
    }
    ...
    if (dskSourcesList && dskStatus.dskSources.length > 0) {
      dskSourcesList.innerHTML = dskStatus.dskSources.map(source => `
      ...
      dskSourcesList.innerHTML = '<p style="color: #888; text-align: center; padding: 12px;">No DSK sources found</p>';
    }
...
function updatePanelConnectionStatus(connected) {
  ...
  if (statusIndicator) {
    statusIndicator.className = connected ? 'status-dot status-connected' : 'status-dot status-disconnected';
  }
...
function clearPanelData() {
  ...
  if (scenesList) scenesList.innerHTML = '<li class="empty-state">Connect to OBS to see scenes</li>';
...
function handleSaveSceneConfig() {
  ...
  const monitorCheckboxes = document.querySelectorAll('#obs-monitor-scenes-list input[type="checkbox"]:checked');
