...
if (titleBoxSize) {
  titleBoxSize.addEventListener('change', function () {
    ...
    document.getElementById('titleBoxSizeDisplay').textContent =
...
if (titleShadow) {
  ...
  if (window.panelStyleManager) {
    ...
    if (titleShadowColorGroup)
      titleShadowColorGroup.style.display = savedTitleShadow ? 'block' : 'none';
    if (titleShadowSizeGroup)
      titleShadowSizeGroup.style.display = savedTitleShadow ? 'block' : 'none';
  }
}
...
if (titleBoxSize) {
  ...
  document.getElementById('titleBoxSizeDisplay').textContent =
    savedTitleBoxSize.charAt(0).toUpperCase() + savedTitleBoxSize.slice(1);
...
if (savedTextShadow) {
  const shadowParts = savedTextShadow.match(
    /(-?\d+)px\s+(-?\d+)px\s+(\d+)px\s+(#[0-9a-f]{6}|rgb\([^)]+\)|rgba\([^)]+\))/i
  );
  if (shadowParts) {
...
if (resetAllSettingsBtn) {
  resetAllSettingsBtn.addEventListener('click', () => {
    if (
      confirm(
        '¿Estás seguro de que quieres resetear TODAS las configuraciones? Esta acción no se puede deshacer.'
      )
    ) {
      localStorage.clear();
    ...
    if (titleShadowConfig) {
      ...
      if (titleShadow) {
        ...
        if (titleShadowConfig.enabled) {
          if (titleShadowColor)
            titleShadowColor.dispatchEvent(new Event('input', { bubbles: true }));
